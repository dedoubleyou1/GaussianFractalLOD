...
\title{Compact Level-of-Detail Object Reconstruction via Coarse-to-Fine Gaussian Splat Priors}
...
\begin{abstract}
A 3D Gaussian can be viewed as a soft, distributional claim about a part of space: an assertion that some amount of occlusion and color exists, approximately in some area, distributed approximately like the Gaussian itself. Under this view, the parameters of a coarse Gaussian are enough to derive a plausible set of finer Gaussians refining the same area. Finer levels are then trained to encode only their corrections to that derivation, kept small enough to compress, while each coarse level remains independently renderable for level-of-detail rendering. We exploit this on bounded objects with known alpha masks, computing the coarsest Gaussian --- a single summary of the entire object --- in closed form from silhouette statistics, then training coarse-to-fine through gradient-driven subdivision into a self-similar hierarchy. On the NeRF Synthetic benchmark, the finest level of our hierarchy matches or exceeds the per-scene PSNR reported in the original 3D Gaussian Splatting work at comparable Gaussian counts, while storing all 13 levels of detail at only a 20\% premium over the cost of the finest level alone --- despite containing 3.1--3.5$\times$ as many Gaussians across the full hierarchy.
\end{abstract}
...
This single observation drives the entire system we propose. Our method is an object reconstruction approach --- restricted, importantly, to bounded objects with clearly defined silhouettes via alpha masks --- that exploits the prior structure of Gaussians at every step:
...
We demonstrate the method on the NeRF Synthetic benchmark, where each object has clean alpha masks. It achieves competitive image quality, produces a usable LOD hierarchy from a single training run, and compresses substantially below flat 3DGS through structural delta storage.
